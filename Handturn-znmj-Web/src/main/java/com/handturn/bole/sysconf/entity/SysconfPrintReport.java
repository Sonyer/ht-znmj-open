package com.handturn.bole.sysconf.entity;

import java.util.Date;

import java.io.Serializable;
import lombok.Data;
import com.wuwenze.poi.annotation.Excel;
import com.wuwenze.poi.annotation.ExcelField;
import com.baomidou.mybatisplus.annotation.IdType;
import com.baomidou.mybatisplus.annotation.TableField;
import com.baomidou.mybatisplus.annotation.TableId;
import com.baomidou.mybatisplus.annotation.TableName;
import com.handturn.bole.common.entity.BasicEntity;

/**
 * 系统配置-打印模板 Entity
 *
 * @author Eric
 * @date 2020-01-31 09:19:11
 */
@Data
@TableName("sysconf_print_report")
@Excel("系统配置-打印模板")
public class SysconfPrintReport extends BasicEntity{

    /**
     * 
     */
    @TableId(value = "id", type = IdType.AUTO)
    private Long id;

    /**
     * 打印编码
     */
    @TableField("print_code")
    @ExcelField(value = "打印编码")
    private String printCode;

    /**
     * 打印名称
     */
    @TableField("print_name")
    @ExcelField(value = "打印名称")
    private String printName;

    /**
     * 报表模板文件名
     */
    @TableField("templet_file_name")
    @ExcelField(value = "报表模板文件名")
    private String templetFileName;

    /**
     * 客户编号
     */
    @TableField("client_code")
    @ExcelField(value = "客户编号")
    private String clientCode;

    /**
     * 客户名称
     */
    @TableField("client_name")
    @ExcelField(value = "客户名称")
    private String clientName;


    /**
     * 仓库编号
     */
    @TableField("warehouse_code")
    @ExcelField(value = "仓库编号")
    private String warehouseCode;

    /**
     * 仓库名称
     */
    @TableField("warehouse_name")
    @ExcelField(value = "仓库名称")
    private String warehouseName;

    /**
     * 组织编号
     */
    @TableField("org_code")
    @ExcelField(value = "组织编号")
    private String orgCode;

    /**
     * 组织名称
     */
    @TableField("org_name")
    @ExcelField(value = "组织名称")
    private String orgName;

    /**
     * 状态: ENABLED-有效;DISABLED-无效
     */
    @TableField("status")
    @ExcelField(value = "状态: ENABLED-有效;DISABLED-无效")
    private String status;

    /**
     * 文件名
     */
    @TableField("file_name")
    @ExcelField(value = "文件名")
    private String fileName;

    /**
     * 文件路径
     */
    @TableField("file_path")
    @ExcelField(value = "文件路径")
    private String filePath;

    /**
     * 备注
     */
    @TableField("remark")
    @ExcelField(value = "备注")
    private String remark;

    /**
     * 是否系统创建(1:是 0:否)，系统创建的记录不能前台修改和删除
     */
    @TableField("is_sys_created")
    @ExcelField(value = "是否系统创建(1:是 0:否)，系统创建的记录不能前台修改和删除")
    private String isSysCreated;


    @Override
    protected Serializable pkVal() {
        return this.getId();
    }
}
